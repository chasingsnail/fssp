...
$.widget( "awsui.combobox_old", {
	...
	_initTrigger: function() {
		if(this.options.trigger!==false){
			...
			this.trigger=$('<a href="javaScript:void"  class="'+this.options.triggerClass+'"> </a>').appendTo(this.wrapper)
			.click(function() {
				self._triggerClick();
			});
	...
	_create: function() {
		...
		if (this.options.hiddenName != null) {
			this.hiddenField=$("<input type='hidden' id='" + this.options.hiddenName + "' name='" + this.options.hiddenName + "'>").insertBefore(this.element);
	...
	_addItemValue:function(data){
		...
		if(this.options.multiple===true){
			...
			v=this.option("value")!=null && this.option("value").length>0?this.option("value")+this.options.seperator+data[this.options.valueField]:data[this.options.valueField];
		}
		this._setvalue(v);
		this._value(label);
	},
