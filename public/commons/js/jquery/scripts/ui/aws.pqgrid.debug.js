...
(function ($) {
	...
	$.paramquery.awsgrid = $.paramquery.awsgrid || {};
...
(function ($) {
	...
	fnPG._regional = {
		...
		strRefresh: "Refresh"
		//,strRpp: "Records per page:{0}"  隐藏设置页数的UI by wzw 201902261352
	};
...
(function ($) {
	...
	fnSB.drag = function (evt) {
		...
		this._trigger("drag", evt, {
			ratio: o.ratio,
			evt: evt  //增加属性判断是通过鼠标drag 201802281657  by wzw
		})
	};
...
(function ($) {
	...
	_pGenerateView.generateView = function (objP) {
		...
		if (objP) {
			...
			$cont.empty();
			if (pqpanes.v) {
				$cont.append(["<div class='aws-grid-cont-inner'>", strTbl, "</div>", "<div class='aws-grid-cont-inner'>", strTbl, "</div>"].join(""))
			} else {
				$cont.append("<div class='aws-grid-cont-inner'>" + strTbl + "</div>")
			}
			if (flexHeight !== "flex") {
				that.iRefresh.refreshScrollbars(); //解决固定高度，竖向滚动条不能及时算出导致横向table与head最后错位问题 20181213201232 by wzw
			}
			...
			if (pqpanes.v) {
				...
				$cont_r.css({
					left: wd - 1,
					width: contWd - wd,
					height: tblHt + 2  //增加2像素，主要用于统计下边框与上边框的显示  201803151837 by wzw
				});
				$tbl_r.css({
					left: (-1 * (lft + wd))
				})
			...
			if (that.totalVisibleRows === 0) {
				$cont.append(o.noRows ? o.noRows({$cont: $cont}) : "<div class='aws-grid-cont-inner aws-grid-norows' >" + o.strNoRows + "</div>"); //实现无数据时显示接口 by wzw 20180507105325
			} else {
				var style = (flexHeight || flexWidth) ? "style='position:relative;'" : "";
				if (pqpanes.h && pqpanes.v) {
					$cont[0].innerHTML = ["<div class='aws-grid-cont-inner'>", strTbl, "</div>", "<div class='aws-grid-cont-inner'>", strTbl, "</div>", "<div class='aws-grid-cont-inner'>", strTbl, "</div>", "<div class='aws-grid-cont-inner'>", strTbl, "</div>"].join("")
				} else {
					if (pqpanes.v) {
						$cont[0].innerHTML = ["<div class='aws-grid-cont-inner' ", style, " >", strTbl, "</div>", "<div class='aws-grid-cont-inner' >", strTbl, "</div>"].join("")
					} else {
						if (pqpanes.h) {
							$cont[0].innerHTML = ["<div class='aws-grid-cont-inner' style='position:static;' >", strTbl, "</div>", "<div class='aws-grid-cont-inner' style='position:static;' >", strTbl, "</div>"].join("")
						} else {
							$cont[0].innerHTML = ["<div class='aws-grid-cont-inner' ", style, " >", strTbl, "</div>"].join("")
						}
					}
				}
			}
	...
	_pGenerateView._generateTables = function (initV, finalV, objP) {
		...
		if (1 === 1) {
			...
			if (that.option().customSet) {   //增加自定义显示列的hook 201803141620 by wzw
				buffer.push("<td style='width:" + (that.option().customSet.width + 17) + "px;'></td>"); //  17是垂直滚动条的宽度
			}
			buffer.push("</tr>")
		}
		this.offsetRow = null;
		for (var row = 0; row <= finalRow; row++) {
			...
			if (GM && rowData.GMRowIndx != null) {   //更改判断方式   防止生成undefined分组头的时候渲染错误问题 201802051629  by wzw
				this._generateTitleRow(GM, rowData, buffer, lastFrozenRow)
			} else {
				if (GM && rowData.groupSummary || rowData.forceSummary) {  //增加forceSummary 强制统计  201802051633 by wzw
	...
	_pGenerateView._renderCell = function (objP, _dataCell) {
		...
		if (_dataCell != undefined) {
			...
			if (type == "checkBoxSelection") {
				dataCell = objP.rowData.AWS_Grid_noCheckbox ? "" : "<input type='checkbox' " + (cellData ? "checked='checked'" : "") + " />" // AWS_Grid_noCheckbox属性禁止选中 201802051641 by wzw
			} else {
				if (type == "detail") {
	...
	_pGenerateView.renderCell = function (objP) {
		...
		var pq_cellattr = that.mergeCellAttr(rowData, column); //兼容colmodel的Attr属性 201802261623 原 rowData.pq_cellattr by wzw;
		if (pq_cellattr) {
		...
		if (column.render) {//移动render的位置，以便传递更多的参数（每个cell下存在a标签部分样式会不生效）  201804031443 by wzw
	...
	_pGenerateView._generateRow = function (rowData, rowIndx, CM, buffer, objP, lastFrozenRow, nestedshow) {
		...
		if (numberCell.show) {
			buffer.push(["<td class='aws-grid-number-cell ui-state-default'>", ((objP) ? "&nbsp;" : (rowIndx + (numberCell.total ? that.rowIndxOffset : 0) + 1)), "</td>"].join(""))  //分页后序号累加  201802101637 by wzw
		}
		var objRender = {
	...
	fn.options = {
		...
		hoverMode: "null",
		_maxColWidth: "100%",
		_minColWidth: 20, // 最小宽度设置为20 （原50）201802051643  by wzw
		minWidth: 100,
		numberCell: {
			width: 30,
			title: "",
	...
	fn._destroy = function () {
		...
		this.element.empty().css("height", "").css("width", "").removeClass("aws-grid ui-widget ui-widget-content ui-corner-all").removeData()
	};
	fn.destroy = function () {
	...
	fn.collapse = function (objP) {
		var that = this,
			...
			postCollapse = function () {
				...
				$icon.attr("title", CP.ctitle);  //增加展开时title 201802051644 by wzw
		...
		if (objP.animate === false) {
			ele.height(36); //收起高度由23改为36 201802051645 by wzw
			postCollapse()
		} else {
			CP.animating = true;
			ele.animate({
				height: "36px"  //收起高度由23改为36 201802051646 by wzw
			}, function () {
				postCollapse()
			})
		}
	...
	fn.expand = function (objP) {
		var that = this,
			...
			postExpand = function () {
				...
				$icon.attr("title", CP.etitle);   //增加收起时title 201802051647 by wzw
	...
	fn._createCollapse = function () {
		...
		if (CP.on) {
			if (!CP.$collapse) {
				CP.$collapse = $(["<div style='float:left;' class='pq-collapse'>", "<span class='ui-icon ui-icon-circle-triangle-n' style='float:left;'", CP.etitle ? (" title = '" + CP.etitle + "'") : "", "></span>", "</div>"].join("")).appendTo(CP.$stripe).mouseover(function (evt) {  //增加收起时title 201802051648 by wzw
		...
		if (CP.toggle) {
			if (!CP.$toggle) {
				CP.$toggle = $(["<div style='float:left;' class='pq-max'>", "<span class='ui-icon ui-icon-arrow-4-diag' style='float:left;' ", CP.ttitle ? (" title = '" + CP.ttitle + "'") : "", "  ></span>", "</div>"].join("")).prependTo(CP.$stripe).mouseover(function (evt) { //增加最大化的title    201802261712 by wzw
					$(this).addClass("ui-state-hover")
				}).mouseout(function (evt) {
					$(this).removeClass("ui-state-hover")
				}).click(function (evt) {
	...
	fn._create = function () {
		...
		element.empty().addClass("aws-grid ui-widget ui-widget-content" + (o.roundCorners ? " ui-corner-all" : "")).html(["<div class='aws-grid-top ui-widget-header", (o.roundCorners ? " ui-corner-top" : ""), "'>", "<div class='aws-grid-title", (o.roundCorners ? " ui-corner-top" : ""), "'>&nbsp;</div>", "</div>", "<div class='aws-grid-center' >", "<div class='pq-header-outer ui-widget-header'>", "</div>", "<div class='aws-grid-cont-outer' >", "<div class='aws-grid-cont' tabindex='0'></div>", "</div>", "</div>", "<div class='aws-grid-bottom ui-widget-header", (o.roundCorners ? " ui-corner-bottom" : ""), "'>", "<div class='aws-grid-footer'>&nbsp;</div>", "</div>"].join(""));
		this._trigger("render", null, {
			dataModel: this.options.dataModel,
			colModel: this.colModel
		});
		this.$top = $("div.aws-grid-top", element);
		this.$title = $("div.aws-grid-title", element);
		if (!o.showTitle) {
			this.$title.css("display", "none")
		}
		this.$toolbar = $("div.aws-grid-toolbar", element);
		this.$grid_inner = $("div.aws-grid-center", element);
		this.$header_o = $("div.pq-header-outer", this.$grid_inner);
		if (!o.showTop) {
			this.$top.css("display", "none")
		}
		this.$bottom = $("div.aws-grid-bottom", element);
		if (!o.showBottom) {
			this.$bottom.css("display", "none")
		}
		this.$footer = $("div.aws-grid-footer", element);
		this.$cont_o = $(".aws-grid-cont-outer", this.$grid_inner);
		var $cont = this.$cont = $("div.aws-grid-cont", this.$grid_inner);
		$(window).bind("resize" + that.eventNamespace + " orientationchange" + that.eventNamespace, function (evt, ui) {
			that.onWindowResize(evt, ui)
		});
		$cont.on("click", "td.aws-grid-cell", function (evt) {
			if ($.data(evt.target, that.widgetName + ".preventClickEvent") === true) {
				return
			}
			if ($(evt.target).closest(".aws-grid")[0] == that.element[0]) {
				return that._onClickCell(evt)
			}
		});
		$cont.on("click", "td.aws-grid-number-cell", function (evt) {  //增加numcellclick事件  201802071818 by wzw
			if (that._onClickNum) {
				if ($.data(evt.target, that.widgetName + ".preventClickEvent") === true) {
					return
				}
				if ($(evt.target).closest(".aws-grid")[0] == that.element[0]) {
					return that._onClickNum(evt)
				}
			}
		});
		if (this.options.rowDrag) {   //增加拖动事件  201805041540 by wzw
		...
		$cont.on("click", "tr.aws-grid-row", function (evt) {
			if ($.data(evt.target, that.widgetName + ".preventClickEvent") === true) {
				return
			}
			if ($(evt.target).closest(".aws-grid")[0] == that.element[0]) {
		...
		$cont.on("contextmenu", "td.aws-grid-cell", function (evt) {
			if ($(evt.target).closest(".aws-grid")[0] == that.element[0]) {
				return that._onRightClickCell(evt)
			}
		});
		$cont.on("contextmenu", "tr.aws-grid-row", function (evt) {
			if ($(evt.target).closest(".aws-grid")[0] == that.element[0]) {
				return that._onRightClickRow(evt)
			}
		});
		$cont.on("dblclick", "td.aws-grid-cell", function (evt) {
			if ($(evt.target).closest(".aws-grid")[0] == that.element[0]) {
				return that._onDblClickCell(evt)
			}
		});
		$cont.on("dblclick", "tr.aws-grid-row", function (evt) {
			if ($(evt.target).closest(".aws-grid")[0] == that.element[0]) {
				return that._onDblClickRow(evt)
			}
		});
		$cont.on("mousedown", "td.aws-grid-cell", function (evt) {
			if ($(evt.target).closest(".aws-grid")[0] == that.element[0]) {
				return that._onCellMouseDown(evt)
			}
		});
		$cont.on("mousedown", "tr.aws-grid-row", function (evt) {
			if ($(evt.target).closest(".aws-grid")[0] == that.element[0]) {
		...
		$cont.on("mouseenter", "td.aws-grid-cell", function (evt) {
			if ($(evt.target).closest(".aws-grid")[0] == that.element[0]) {
				return that._onCellMouseEnter(evt, $(this))
			}
		});
		$cont.on("mouseenter", "tr.aws-grid-row", function (evt) {
			if ($(evt.target).closest(".aws-grid")[0] == that.element[0]) {
				return that._onRowMouseEnter(evt, $(this))
			}
		});
		$cont.on("mouseleave", "td.aws-grid-cell", function (evt) {
			if ($(evt.target).closest(".aws-grid")[0] == that.element[0]) {
				return that._onCellMouseLeave(evt, $(this))
			}
		});
		$cont.on("mouseleave", "tr.aws-grid-row", function (evt) {
			if ($(evt.target).closest(".aws-grid")[0] == that.element[0]) {
	...
	fn._mouseCapture = function (evt) {
		...
		if ($(evt.target).closest(".aws-grid")[0] == this.element[0]) {
	...
	fn._mouseDown = function (evt) {
		...
		return true;
		//this._super(evt);  201807131010 by wzw      此调用会阻止选择文字高亮问题，造成其他后果暂时无法估计
	};
	...
	fn._onMouseWheel = function (evt) {
		...
		var num = 0,
			...
			wheelDeltaX = evt.wheelDeltaX ? evt.wheelDeltaX : (evt.deltaX == 0 ? 0 : (evt.deltaX > 0 ? -120 : 120)),//左右x120为标准 滑轮事件兼容 by wzw  201805071714
			wheelDeltaY = evt.wheelDeltaY ? evt.wheelDeltaY : (evt.deltaY == 0 ? 0 : (evt.deltaY > 0 ? -11 : 11)),//上下y11为标准 滑轮事件兼容 by wzw  201805071714
			wheelDelta = evt.wheelDelta ? evt.wheelDelta : (Math.abs(wheelDeltaY) > 0 ? wheelDeltaY : wheelDeltaX); //滑轮事件兼容 by wzw  201805071714
		...
		if ((horizontal && o.virtualX) || (!horizontal && o.virtualY)) {
			...
			//TODO 虚拟滚动时 如果到顶不阻止滑轮事件  201806271810 by wzw
		} else {
			...
			if (new_ratio == 0 || new_ratio == 1) {  //如果到顶不阻止滑轮事件 201806271810 by wzw
				return true;
			}
		}
		return false
		...
		if (allowInvalid || warn) {
			...
			if (!_valid) {
				...
				if (cell && cell.$cell) {
					...
					$cell.attr("title", msg);
					try {
						$cell.tooltipgrid("destroy")//避免平台tooltip冲突把$.ui.tooltip改为$.ui.tooltipgrid 201802051723 by wzw
					} catch (ex) {
					}
					$cell.tooltipgrid({//避免平台tooltip冲突把$.ui.tooltip改为$.ui.tooltipgrid 201802051723 by wzw
						position: {
							my: "left center+5",
							at: "right center"
						},
					...
					}).tooltipgrid("open")//避免平台tooltip冲突把$.ui.tooltip改为$.ui.tooltipgrid 201802051723 by wzw
				}
				return objvalid
			}
			if (valueDef) {
	...
	fn._onContMouseDown = function (evt) {
		...
		var $target = $(evt.target),
			$td = $target.closest(".aws-grid-cell"),
			$tr = $target.closest(".aws-grid-row");
	...
	fn.generateLoading = function () {
		...
		$(["<div class='pq-loading-bg'></div><div class='pq-loading-mask ui-state-highlight'><div><img src='../commons/img/waiting.gif' style='width:30px;height:30px;'/><span style='margin-left: 13px;font-size:14px;'>" + 正在加载 + "...</span>", "</div></div>"].join("")).appendTo(this.$loading);
	...
	fn.refreshDataFromDataModel = function () {
		...
		if (paging && paging == "local") {
			...
			if (paging == "remote") {
				...
				this.pdata = DMdata;//.slice(0, endIndx)  屏蔽：顺序增加不再有翻页限制  201806121411 by wzw
			} else {
		...
		if (paging == "local" || paging == "remote") {
			this.rowIndxOffset = (PM.rPP * ((PM.curPage ? PM.curPage : 1) - 1))  //curPage至少是1，否则ajax新建第一条时会计算出负数 201802051649  by wzw
		}
	};
		...
		this.xhr = $.ajax({
			...
			success: function (responseObj, textStatus, jqXHR) {
				...
				if (objP && objP.callback) {
					objP.callback(responseObj); //回调参数 by wzw 201805071825
				}
	...
	fn.refreshDataAndView = function (objP) {
		...
		if (DM.location == "remote") {
			var self = this;
			this.remoteRequest({
				callback: function (r) { //回调参数 by wzw 201805071825
					self._onDataAvailable(objP)
					if (objP && objP.callback) { //增加刷新回调 201803141801 by wzw
						objP.callback(r);//回调参数 by wzw 201805071825
					}
				}
			})
		} else {
	...
	fn._generateCellRowOutline = function () {
		...
		$td.css("height", $td.height() - 1); //编辑框的高度减去1px 201802271124 by wzw
		$td.empty();
			...
			for (var dataIndx in newRow) {
				...
				if (column) {
					...
					var dataType = column.dataType,
						newVal = newRow[dataIndx], //getValueFromDataType(newRow[dataIndx], dataType),  201804181416 不再转换类型 by wzw
		...
		for (var i = 0; i < rowListLen; i++) {
			...
			if (type == "update") {
				...
				if (type == "add") {
					...
					//if (paging == "remote") { //屏蔽代码：新建不再计算总数  201802051653  by wzw
					//	PM.totalRecords++
					//}
				} else {
					if (type == "delete") {
	...
	fn.refreshCell = function (obj) {
		...
		if (colIndx == null) { //空指针  201802051655 by wzw
			return
		}
		var $td = this.getCell({
				all: true,
	...
	fn._editCell = function (objP) {
		...
		var $tdChirldren = this.getCell(objP).children();// 编辑事件提前获得tdChildren 201802051656 by wzw
		...
		if (typeof edtype == "function") {
			...
			this.customQuitEvent(inp, this, {$cell: $cell, $tdChirldren: $tdChirldren, rowIndx: rowIndx, rowIndxPage: rowIndxPage, colIndx: colIndx}); // 编辑事件 201802051658 by wzw
		} else {
			if (edtype == "checkbox") {
		...
		var that = this;
		var $focus = $cell.find(".pq-editor-focus"), // 由children方法改为find ，适应多层结构  201805081419 by wzw
			FK = EM.filterKeys,
		...
		$focus.data({
			...
			if (!that.saveEditCell({
				...
				if (!force && validateOnBlur || that.options.forceQuitEdit === false) { //一个扩展属性，防止blur后校验不通过但会强制退出编辑 201803201022 by wzw
	...
	fn.getCellHeader = function (obj) {
		var colIndx = obj.colIndx,
			...
			rowSpan = this.colModel[colIndx].rowSpan,//多级表头时正确寻找header位置 201804021659 by wzw
			$tbl = this.$tbl_header,
		...
		if ($tbl != undefined) {
			...
			var $td = $tbl.children().children("tr.aws-grid-title-row").eq(this.depth - rowSpan).children("td[pq-col-indx=" + colIndx + "]") //多级表头时正确寻找header位置 201804021659 by wzw
		}
	...
	fn.getEditCellData = function () {
		...
		if (typeof getData == "function") {
			...
			$(document).off("click" + this.eventNamespace + "QEMode");  // 注销编辑事件 201802051700 by wzw
			this.element.off(this.widgetEventPrefix + "cellclick" + this.eventNamespace + "QEMode");  // 注销编辑事件 201802051701 by wzw
		} else {
			var edtype = editor.type;
			if (edtype == "checkbox") {
	...
	fn.getCellIndices = function (objP) {
		var $td = objP.$td;
		if ($td == null || $td.length == 0 || $td.closest(".aws-grid")[0] != this.element[0]) {
			return {
				rowIndxPage: null,
				colIndx: null
			}
	...
	fn._onKeyPressDown = function (evt) {
		var $header = $(evt.target).closest(".aws-grid-header");
		if ($header.length > 0) {
	...
	_pKeyNav.incrPageSize = function () {
		var that = this.that,
			$tbl = that.$tbl,
			$trs = $tbl.children("tbody").children("tr.aws-grid-row").not(".pq-group-row,.pq-summary-row"),
	...
	_pKeyNav.pageNonVirtual = function (rowIndxPage, type) {
		...
		var htTotal = 0,
			counter = 0,
			tr, $tr_prevAll = $($tr[0])[type]("tr.aws-grid-row"),
	...
	fn._calcCurPosFromRowIndxPage = function (rowIndxPage) {
		...
		for (var i = freezeRows, len = data.length; i < len; i++) {
			var rowData = data[i];
			if (GM && (rowData.groupSummary || rowData.groupTitle)) {
			} else {
				if (j == rowIndxPage) {
					break
				}
	...
	$.paramquery.awsgrid.calcWidthCols = calcWidthCols;
	...
	$.widget("paramquery._awsGrid", $.ui.mouse, fn)
})(jQuery);
(function ($) {
	var calcWidthCols = $.paramquery.awsgrid.calcWidthCols;
	var fn = $.paramquery._awsGrid.prototype;
	...
	_p.createHeader = function () {
		...
		var tblClass = "aws-grid-header-table ";
		if (hwrap) {
			tblClass += "pq-wrap "
		} else {
			tblClass += "pq-no-wrap "
		...
		if (depth >= 1) {
			...
			if (that.option().customSet) { //增加自定义显示列的hook 201803141620 by wzw
				buffer.push("<td style='width: " + that.option().customSet.width + "px;' ></td>")
			}
			buffer.push("</tr>")
		}
		var const_cls = "aws-grid-col ";
		for (var row = 0; row < depth; row++) {
			buffer.push("<tr class='aws-grid-title-row'>");
			if (row == 0 && numberCell.show) {
				buffer.push(["<td pq-col-indx='-1' class='aws-grid-number-col' rowspan='", depth, "'>", "<div class='pq-td-div'>", numberCell.title ? numberCell.title : "&nbsp;", "</div></td>"].join(""))
			}
			for (var col = 0; col <= finalH; col++) {
				if (col < initH && col >= freezeCols && virtualX) {
					col = initH;
			...
			if (that.option().customSet) { //增加自定义显示列的hook 201803141620 by wzw
				if (row == 0) {
					buffer.push("<td id='grid_custom_set_tdwdm' rowspan='" + depth + "'>" + that.option().customSet.getHtml() + "</td>");
				}
			}
			buffer.push("</tr>")
		}
		...
		$header_o.empty();
		if (pqpanes.vH) {
			$header_o.append(["<span class='aws-grid-header aws-grid-header-left ui-state-default'>", str, "</span>", "<span class='aws-grid-header ui-state-default'>", str, "</span>"].join(""))
		} else {
			$header_o.append(["<span class='aws-grid-header ui-state-default'>", str, "</span>"].join(""))
		}
		var $header = that.$header = $(".aws-grid-header", $header_o);
	...
	_p._onHeaderClick = function (evt) {
		...
		var $target = $(evt.target);
		var $td = $target.closest("td.aws-grid-col");
		if ($td.length) {
	...
	_p.createHeaderCell = function (row, col, headerCells, buffer, const_cls, freezeCols, initH, depth) {
		...
		if (column.colModel == null || column.colModel.length == 0) {
			cls += " aws-grid-col-leaf"
		}
		var rowIndxDD = "pq-row-indx=" + row;
		var colIndxDD = "pq-col-indx=" + col;
		var sortdefault = column.sortable ? "sortdefault" : "";// 增加默认sort样式 201804041757 by wzw
		buffer.push(["<td ", colIndx, " ", colIndxDD, " ", rowIndxDD, " ", " class='", cls, "' rowspan=", column.rowSpan, " colspan=", colSpan, ">", "<div class='pq-td-div'>", title, "<span class='pq-col-sort-icon ", sortdefault, "'>&nbsp;</span></div></td>"].join("")) // 增加默认sort样式 201804041757 by wzw
	};
	...
	_p._refreshResizeColumn = function (initH, finalH, model) {
		...
		if (numberCell.show) {
			...
			if (numberCell.resizable) {
				lft = lftCol - 5;
				buffer1.push("<div pq-col-indx='-1' style='left:", lft, "px;'", " class='aws-grid-col-resize-handle'>&nbsp;</div>")
			}
		}
		for (var i = 0; i <= finalH; i++) {
			...
			if ((column.resizable !== false) || ficon) {
				...
				lft = lftCol - 5;
				buffer.push("<div pq-col-indx='", i, "' style='left:", lft, "px;'", " class='aws-grid-col-resize-handle'>&nbsp;</div>")
			}
		}
		if (buffer2.length) {
	...
	_p.refreshHeaderSortIcons = function () {
		...
		var $tds = $header.find(".aws-grid-col-leaf");
		...
		for (var i = 0; i < sorters.length; i++) {
			...
			$header.find(".aws-grid-col-leaf[pq-col-indx=" + colIndx + "]").addClass(addClass);
			$header.find(".aws-grid-col-leaf[pq-col-indx=" + colIndx + "] .pq-col-sort-icon").addClass(cls2).removeClass("sortdefault")// 增加默认sort样式 201804041757 by wzw
		}
	};
	...
	_pResizeColumns._getDragHelper = function (evt) {
		var that = this.that,
			...
			freezeCols = parseInt(o.freezeCols),
			$target = $(evt.target);
		this.$cl = $("<div class='aws-grid-drag-bar'></div>").appendTo(that.$grid_inner);
		this.$clleft = $("<div class='aws-grid-drag-bar'></div>").appendTo(that.$grid_inner);
	...
	_pResizeColumns._setDragLimits = function (colIndx) {
		...
		var $pQuery_drag = $head.find("div.aws-grid-col-resize-handle[pq-col-indx=" + colIndx + "]");
	...
	_pResizeColumns.resizeStop = function (evt, ui, drag_left) {
		...
		if (colIndx == -1) {
			...
			var oldWidth = parseInt(column.width),
				newWidth = oldWidth + dx;
			column.width = column.baseWidth = newWidth;   //防止分辨率过小后自适应的临时宽度覆盖原始宽度 by wzw 201901301524
	...
	var cDragColumns = function (that) {
		...
		if (dragColumns && dragColumns.enabled) {
			that.$header_o.on("mousedown", "td.aws-grid-col", function (evt, ui) {
	...
	_pDragColumns.setDraggables = function (evt, ui) {
		...
		$td.draggable({
			...
			helper: function () {
				...
				that.$header.find(".aws-grid-col-resize-handle").hide();
			...
			stop: function (evt, ui) {
				self.status = "stop";
				that.$header.find(".aws-grid-col-resize-handle").show();
				self.hideArrows()
			}
		})
	};
...
(function ($) {
	...
	_pHierarchy.tableWidthChange = function () {
		...
		for (var i = 0, tdLen = $tds.length; i < tdLen; i++) {
			var td = $tds[i],
				$td = $(td);
			var $grids = $td.find(".aws-grid");
			for (var j = 0, gridLen = $grids.length; j < gridLen; j++) {
				var $grid = $($grids[j]);
				if ($grid.is(":visible")) {
					$grid.awsGrid("onWindowResize")
				}
			}
		}
		this.refreshComplete = true
	...
	_pHierarchy.aftertable = function ($trs) {
		var that = this.that,
			...
			afterDetail = that.options.detailModel.bind,//增加bind事件，在append之后执行  201804201107 by wzw
			data = that.pdata;
		if (!this.refreshComplete) {
			return
		}
		...
		for (var i = 0, trLen = $trs.length; i < trLen; i++) {
			...
			$td.append($detail);
			if (typeof afterDetail == "function") {//增加bind事件，在append之后执行  201804201107 by wzw
				afterDetail.call(that.element[0], {
					rowData: rowData,
					$trs: $trs
				});
			}
			$trs.eq(0).find("td").height($td.outerHeight() - 9); //解决展开后左右高度不一致问题 201804201018  by wzw
			var $grids = $td.find(".aws-grid");
			for (var j = 0, gridLen = $grids.length; j < gridLen; j++) {
				var $grid = $($grids[j]);
				if (newCreate) {
...
(function ($) {
	...
	_pRefresh.refreshColumnWidths = function () {
		...
		var customSetCellWidth = 0;
		if (that.option().customSet) { //增加自定义显示列的hook 201803141620 by wzw
			customSetCellWidth = that.option().customSet.width;
		}
		var availWidth = flexWidth ? null : (contWd - sbWidth - numberCellWidth - customSetCellWidth),
		...
		for (var i = 0; i < CMLength; i++) {
			...
			var colWidth = column.baseWidth ? column.baseWidth : (column.baseWidth = column.width),  //防止分辨率过小后自适应的临时宽度覆盖原始宽度 by wzw 201901301524
	...
	_pRefresh._refreshFrozenLine = function () {
		...
		if (freezeCols) {
			...
			if (isNaN(lft) || lft === 0) {
				...
				if (lft > 0 && numberCell.show && lft === numberCell.width) {
				} else {
					that.$freezeLine = $(["<div class='aws-grid-vert-frozen-line' ", " style = 'height:100%;top:0;left:", lft, "px;' >", "</div>"].join("")).appendTo($container)
				}
			}
		}
	};
	...
	_pRefresh.setContAndHeaderHeight = function () {
		...
		if ($header && $header.length) {
			if ($header.length > 1) {
				...
				if (htHD0 !== htHD1) {
					var $tr0 = $($header[0]).find(".aws-grid-header-search-row"),
						$tr1 = $($header[1]).find(".aws-grid-header-search-row");
					if ($tr0.length > 0) {   //解决ie8下空指针  （htHD0余htHD1高度有可能不一致） 201804031821 by wzw
						$tr0.css("height", "");
						$tr1.css("height", "");
						htHD0 = $header[0].offsetHeight;
						htHD1 = $header[1].offsetHeight;
			...
			that.$header_o.height(htHD - 1); //减去一个像素 201804081841 by wzw
			this.headerHt = htHD
		} else {
	...
	_pRefresh.calcColsOutsideCont = function (model) {
		...
		if (that.option().customSet) { //增加自定义显示列的hook 201803141620 by wzw
			tblWd += that.option().customSet.width;
		}
	...
	_pRefresh._refreshTableWidths = function (CMOld, objP) {
		var that = this.that,
			...
			$draggables = header ? that.$header.find("div.aws-grid-col-resize-handle") : null,
...
(function ($) {
	...
	fn.getFocusElement = function () {
		var ae;
		try {
			ae = document.activeElement; // 201804181730 解决ie8兼容模式莫名报错问题  by wzw
		} catch (e) {
		}
		if (ae) {
			var $ae = $(ae),
				$grid = $ae.closest(".aws-grid");
			if ($grid.length) {
	...
	var cHeaderSearch = function (that) {
		...
		that.element.on(widgetEventPrefix + "headerkeydown" + eventNamespace, function (evt, ui) {
			if (self.belongs(evt)) {
				...
				if ($src.hasClass("aws-grid-hd-search-field")) {
	...
	_pHeaderSearch.get$Ele = function (colIndx, dataIndx) {
		var that = this.that,
			...
			$inp, selector = ".aws-grid-hd-search-field[name='" + dataIndx + "']",
	...
	_pHeaderSearch._bindFocus = function () {
		...
		function handleFocus(e) {
			...
			var $target = $(e.target),
				$inp = $target.closest(".aws-grid-hd-search-field"),
		...
		var $trs = that.$header.find(".aws-grid-header-search-row");
		for (var i = 0; i < $trs.length; i++) {
	...
	_pHeaderSearch.createDOM = function (buffer, td_const_cls) {
		...
		buffer.push("<tr class='aws-grid-header-search-row'>");
		if (numberCell.show) {
			buffer.push(["<td pq-col-indx='-1' class='aws-grid-number-col' rowspan='1'>", "<div class='pq-td-div'>&nbsp;</div></td>"].join(""))
		}
		for (var i = 0; i <= finalH; i++) {
			if (i < initH && i >= freezeCols && virtualX) {
				i = initH;
			...
			if (filter) {
				var dataIndx = column.dataIndx,
					...
					cls = filter.cls,
					cls = "aws-grid-hd-search-field " + (cls ? cls : ""),
					style = filter.style,
					style = style ? style : "",
					attr = filter.attr,
					attr = attr ? attr : "",
	...
	_pGenerateView._generateTitleRow = function (GM, rowObj, buffer, lastFrozenRow) {
		var that = this.that,
			...
			GMTitle = GM.title,
			GMNewGroupTitle = GM.newGroupTitle, //增加属性，如果新建的数据分组提示（就是把总数替换一下） 201802051703 by wzw
			GMIcon = GM.icon,
		...
		var row_cls = 'pq-group-row aws-grid-row",(lastFrozenRow?" pq-last-frozen-row":""),"',
		...
		if (numberCell.show) {
			buffer.push("<td class='aws-grid-number-cell ui-state-default'>&nbsp;</td>")
		}
		var icon = GMIcon[0];
		if (rowObj.collapsed) {
			icon = GMIcon[1]
		}
		buffer.push("<td class='aws-grid-cell' colSpan='100' >", "<div class='pq-td-div' style='margin-left:", (groupLevel * 16), "px;'>", "<span class='ui-icon ", icon, "'></span>", groupTitle, "</div></td>");
		buffer.push("</tr>")
	};
	_pGenerateView._generateSummaryRow = function (GM, rowObj, thisColModel, buffer, lastFrozenRow) {
		var level = rowObj.level,
			...
			row_cls = "pq-summary-row aws-grid-row" + (lastFrozenRow ? " pq-last-frozen-row" : "") + (rowObj.index == null || rowObj.index == 0 ? " first-summary" : ""), //增加index参数，第一行数据样样式  by wzwtest
			summaryCls = GM ? GM.summaryCls : null;
		...
		buffer.push(row_str);
		if (numberCell.show) {
			buffer.push(["<td class='aws-grid-number-cell ui-state-default'>", rowObj && rowObj.cellTitle ? rowObj.cellTitle : "&nbsp;", "</td>"].join("")) //增加参数cellTitle属性 201802051636 by wzw
		}
		for (var col = 0; col <= finalH; col++) {
			if (col < initH && col >= freezeCols && virtualX) {
				col = initH;
			...
			var strStyle = "";
			var pq_cellattr = that.mergeCellAttr(groupRowData, column), attr = ""; //统计兼容colmodel的Attr属性start 201803081534
			if (pq_cellattr) {
				var cellattr = pq_cellattr[dataIndx];
				if (cellattr) {
					for (var key in cellattr) {
			...
			}//统计兼容colmodel的Attr属性end 201803081534
			var cls = const_cls;
			if (column.align == "right") {
				cls += " pq-align-right"
			} else {
			...
			var valCell = groupRowData[dataIndx],
				title;
			if (valCell && column.summary && (title = column.summary.title) && (title = title[level])) {
			...
			var str = ["<td class='", cls, "' ", attr, " >", valCell, "</td>"].join("");//统计兼容colmodel的Attr属性 201803081534
			buffer.push(str)
		}
		buffer.push("</tr>");
		return buffer
	...
	_pGenerateView._generateDetailRow = function (rowData, rowIndx, thisColModel, buffer, objP, lastFrozenRow) {
		var row_cls = "aws-grid-row pq-detail-child";
		if (lastFrozenRow) {
			row_cls += " pq-last-frozen-row"
		}
		var that = this.that,
		...
		if (numberCell.show) {
			buffer.push(["<td class='aws-grid-number-cell ui-state-default'>", "&nbsp;</td>"].join(""))
		}
	...
	_pGroupView.showHideRows = function (initIndx, level, hide) {
		...
		for (var i = initIndx, len = data.length; i < len; i++) {
			...
			if (rowData.groupSummary) {
				...
				if (rowData.groupTitle != null) { //解决分组title为空串时判断不正确问题 201806121404 by wzw
					if (rowObj.collapsed) {
						arr.push({
							indx: i,
							level: rowObj.level
	...
	_pGroupView._groupData = function () {
		var that = this.that,
			...
			PM = thisOptions.pageModel,
			CM = that.colModel,
			rowOffset = (PM.type) ? (((PM.curPage < 1 ? 1 : PM.curPage) - 1) * PM.rPP) : 0,  // curPage 为 0 时导致rowOffset为负数  201802071613 by wzw
			CMLength = CM.length,
		...
		if (GM && data && data.length > 0) {
			var dataGM = [],
				...
				cols = [];
			for (var u = 0; u < GMLength; u++) {
				prevGroupVal[u] = "g_p11q_Null"; //加入一个初始值避免“”串造成分组空串时会空指针 201802051704 by wzw
				groupVal[u] = "";
				cols[u] = {}
			}
			for (var i = 0, len = data.length; i <= len; i++) {
				...
				for (var u = 0; u < GMLength; u++) {
					groupVal[u] = (i < len) ? $.trim(rowData[GMdataIndx[u]]) : "g_p11q_Null";//加入一个初始值避免“”串造成分组不计算总个数 201802051705 by wzw
				...
				if (changeGroup) {
					...
					for (var m = changeGroupIndx; m < GMLength; m++) {
						dataGM.push({
							...
							level: m,
							isCreate: that.isCreateFunc && that.isCreateFunc(rowData), //判断是否新建数据（针对aws表单）201802051707  by wzw
							nextRowData: rowData,
	...
	fn._createToolbar = function () {
		...
		if (toolbar) {
			var tb = toolbar,
				...
				attr = attr ? attr : "",
				items = tb.items;
			var $toolbar = $("<div class='" + cls + "' style='" + style + "' " + attr + " ></div>").appendTo($(".aws-grid-top", this.element));
			$toolbar.pqToolbar({
				items: items,
				gridInstance: this
			});
	...
	fn.exportExcel = function (obj) {
		obj.format = "xml";
		return $.paramquery.awsgrid.exportToExcel.call(this, obj)
	};
	...
	fn.getData = function (objP) {
		if (objP == null) { //兼容旧方法 201802051709 by wzw
			return this.options.dataModel.data || [];
		}
	...
	$.widget("paramquery.awsGrid", $.paramquery._awsGrid, fn);
	$.paramquery.awsGrid.regional = {};
	$.paramquery.awsGrid.regional.en = fn._regional
})(jQuery);
(function ($) {
	$.paramquery = ($.paramquery == null) ? {} : $.paramquery;
	$.paramquery.awsgrid = ($.paramquery.awsgrid == null) ? {} : $.paramquery.awsgrid;
	$.paramquery.awsgrid.exportToExcel = function (obj) {
		var that = this,
			urlPost = (obj.urlPost === undefined) ? obj.url : obj.urlPost,
			urlExcel = obj.url,
			sheetName = (obj.sheetName == null) ? "awsGrid" : obj.sheetName,
			format = obj.format,
...
(function ($) {
	var pq_options = $.paramquery.awsGrid.prototype.options;
	var trackModel = {
		on: false,
	...
	_pUCData.update = function (objP) {
		...
		if (that.isCreateFunc) { // 判断是否新建的数据 自定义判断 201802051712  by wzw
			if (that.isCreateFunc(rowData, objP)) {
				return;
			}
		}
		for (var i = 0, len = udata.length; i < len; i++) {
			...
			if (rec.rowData == rowData) {
				...
				for (var dataIndx in newRow) {
					var column = columns[dataIndx];
					if (column == null) { // 有可能dataIndx没有colModle 空指针 201802051713 by wzw
						continue;
					}
					var dataType = column.dataType,
	...
	fnGrid.getRowData = function (obj) {
		obj = typeof obj == "number" || typeof obj == "string" ? {rowIndxPage: parseInt(obj, 10)} : obj; //兼容旧版本 201802051715 by wzw
		if (!obj) {
			return null
		}
	...
	fnGrid.addRow = function (objP) {
		if (arguments && arguments.length > 1) {  //兼容旧版本 201804241645 by wzw
			this.addRowCompatibleBefore(arguments);
			return;
		}
		if (objP.rowData && objP.rowData.length != null) {  //支持批量添加 201804241646 by wzw
			this.addRowsCustom(objP);
			return;
		}
		var that = this,
			rowData = objP.rowData || objP.row,
			rowList = objP.rowList,//增加一次添加多个 by wzw
			rowIndx = objP.rowIndx,
			rowIndxPage = objP.rowIndxPage,
			offset = that.rowIndxOffset,
			rowIndx = (rowIndxPage != null) ? rowIndxPage + offset : rowIndx,
		...
		var ret = this._digestData({
			...
			track: objP.track,
			checkEditable: objP.checkEditable,
			rowList: rowList ? rowList : [{  //支持批量添加 201804241646 by wzw
				newRow: rowData,
				rowIndx: rowIndx,
				type: "add"
			}]
	...
	fnTB._create = function () {
		...
		for (var i = 0, len = items.length; i < len; i++) {
			var item = items[i],
				...
				listeners = listener ? [listener] : (item.listeners ? item.listeners : [{click: item.render}]),  //兼容旧版本的render 201802051716 by wzw
				itemcls = item.cls,
				cls = (type == "button" ? "button " : "") + (itemcls ? itemcls : ""),  //更改平台button样式 201802051717 by wzw
				itemstyle = item.style,
				style = itemstyle ? 'style="' + itemstyle + '"' : "",
				itemattr = item.attr,
				attr = itemattr ? itemattr : "",
				$ctrl;
			if (typeof attr == "object") {  //支持json参数 201803231429 by wzw
				var attrstr = "";
				for (var s in attr) {
					attrstr += " " + s + "=\"" + attr[s] + "\"";
				}
				...
				if (type == "checkbox") {
					...
					if (type == "separator") {
						...
						if (type == "button") {
							...
							var eleObj = element;
							if (item.group != null) {
								eleObj = element.find("span[group=" + item.group + "]");
								if (eleObj.length == 0) {
									eleObj = $("<span group='" + item.group + "' class='button-group'></span>").appendTo(element);
								}
							}
							$ctrl = $("<button type='button' class='" + cls + "' " + attr + " " + style + ">" + text + "</button>").appendTo(eleObj); //不调用jqueryui的button方法   .button(options)  201802051718 by wzw
						} else {
							if (type == "select") {
								var options = item.options ? item.options : [];
								if (typeof options === "function") {
...
(function ($) {
	...
	_pSelection.getNewRowSel = function () {
		...
		if ($ae && $ae.hasClass("aws-grid-row")) {
			var $grid = $ae.closest(".aws-grid");
	...
	_pSelection.getNewCellSel = function () {
		var that = this.that,
			$ae = document.activeElement ? $(document.activeElement) : null;
		if ($ae && $ae.hasClass("aws-grid-cell")) {
			var $grid = $ae.closest(".aws-grid");
			if ($grid[0] == that.element[0]) {
				var objP = that.getCellIndices({
					$td: $ae
	...
	_pR._add = function (objP) {
		...
		if (objP.focus !== false && inView) {
			...
			if ($tr) {
				$tr = $($tr[0]);
				$tr.attr("tabindex", "0");
				if ($.browser.isIE8) { //ie8 下修改第一行 跳动问题 201802051719 by wzw
...
(function ($) {
	...
	_pCheckBC.refreshHeader = function (evt, ui) {
		...
		$td.off("click").click(function (evt) { //解决多表头绑定事件过多，导致全选慢 by wzw 201903301452
...
(function ($) {
	...
	_pMouseSelection._beforeTableView = function (evt, ui) {
		...
		if (objFE && objFE.$grid) {
			var $ae = objFE.$ae;
			if ($ae.hasClass("aws-grid-row")) {
				var obj = that.getRowIndx({
					$tr: $ae
				});
				this.lastFocus = obj
			} else {
				if ($ae.hasClass("aws-grid-cell")) {
					var obj = that.getCellIndices({
						$td: $ae
					});
					this.lastFocus = obj
	...
	_pMouseSelection._onCellMouseDown = function (evt, ui) {
		...
		if (evt.shiftKey) {
			...
			if (mode != "single") { //不再限制于ctrl进行多选 (evt.ctrlKey || evt.metaKey) &&  201802051721 by wzw
	...
	_pMouseSelection._onRowMouseDown = function (evt, ui) {
		...
		if (evt.shiftKey) {
			...
			if (that._trigger("beforeSelection", that.event, {data: ui.dataModel.data, rowIndx: rowIndx, rowIndxPage: ui.rowIndxPage}) === false) { //增加选中前事件 201802051652 by wzw
				return false
			}
			if (mode != "single") { //不再限制于ctrl进行多选 (evt.ctrlKey || evt.metaKey) &&  2018020517212 by wzw
...
(function ($) {
	var iExcel = null,
		...
		copyProgress = false,
		gMemory = "",
		_pgrid = $.paramquery.awsGrid.prototype,
	...
	_pExcel.createClipBoard = function () {
		var $div = $("#aws-grid-excel-div"),
			CPM = this.that.options.copyModel,
			$text = $("#aws-grid-excel");
		if ($text.length == 0) {
			$div = $("<div id='aws-grid-excel-div'  style='position:fixed;top:20px;left:20px;height:1px;width:1px;overflow:hidden;z-index:" + CPM.zIndex + ";'/>").appendTo(document.body);
			$text = $("<textarea id='aws-grid-excel' autocomplete='off'  style='overflow:hidden;height:10000px;width:10000px;opacity:0' />").appendTo($div);
			$text.css({
				opacity: 0
			})
		}
	...
	$(document).unbind(".pqExcel").bind("keydown.pqExcel", function (evt) {
		if (evt.ctrlKey || evt.metaKey) {
			var $ae = $(evt.target);
			if (!$ae.hasClass("aws-grid-row") && !$ae.hasClass("aws-grid-cell") && !$ae.is("#aws-grid-excel") && !$ae.is("div.aws-grid-cont")) {
				return
			}
			var $grid = $ae.closest(".aws-grid");
			if (iExcel || ($ae.length && $grid.length)) {
				if (!iExcel) {
					try {
						var that = $grid.awsGrid("getInstance").grid
					} catch (ex) {
						return true
					}
...
(function ($) {
	if ($.ui.autocomplete) {  //避免平台menu的冲突加判断(不引用jqueryui的autocomplete和menu) 201802051722 by wzw
	...
	var fn = {
		options: {
			...
			content: function () {
				var $td = $(this),
					$grid = $td.closest(".aws-grid"),
					grid = $grid.awsGrid("getInstance").grid,
	...
	fn._create = function () {
		...
		$ele.on("pqtooltipopen" + eventNamespace, function (evt, ui) {
			...
			if ($grid.is(".aws-grid")) {
				var obj = $grid.awsGrid("getCellIndices", {
						$td: $td
					}),
					rowIndx = obj.rowIndx,
					dataIndx = obj.dataIndx,
					a, rowData = $grid.awsGrid("getRowData", {
						rowIndx: rowIndx
					});
	...
	$.widget("paramquery.pqTooltip", $.ui.tooltipgrid, fn) //避免平台tooltip冲突把$.ui.tooltip改为$.ui.tooltipgrid 201802051723 by wzw
})(jQuery);