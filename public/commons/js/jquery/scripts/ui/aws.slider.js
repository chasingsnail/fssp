 ...
 * author zhangz
 */
var tip = $("#slider-tooltip");
if(tip.length == 0){
	tip = $("<div id='slider-tooltip' class='slider-tooltip'><div class='slider-tooltip-content'></div></div>");
}
 (function($) {
	$.fn.slider = function(options){
			this.hFlag = false;
			if(!arguments.length>0){
				options = {horizontal:true};//空参数情况
			}else{
			...
			return sliderInit(this,options);
		function sliderInit(obj,options){
			var silder_bar = document.createElement('div');
			if(!!options.horizontal){
				silder_bar.setAttribute('class','ui-slider-bar'); 
			}else{
				$(obj).addClass('ui-slider-vertical');
				silder_bar.setAttribute('class','ui-slider-bar ui-slider-bar-vertical'); 
			}
			$(obj).append(tip);
			tip.hide();
			$(obj).append(silder_bar)
			var arrow = "<span class='slider-tooltip-arrow'></span>"+
						"<span class='slider-tooltip-arrow-inner'></span><span class='slider-value'>0</span>";
			tip.find(".slider-tooltip-content").html(arrow);
			tip.css({
					color: "rgb(68, 68, 68)",
					border : "1px solid rgb(149, 184, 231)",
					background: "rgb(255, 255, 255)"
			});
			var min = options.min && typeof options.min=='number'?  options.min :0 ;
			var max = options.max && typeof options.max=='number'?  options.max :0 ;
			if(max<min){
				alert("parameter error: please check params 'min' or 'max'")
			}
			var slider = new Slider(obj.get(0),silder_bar,options);
			return slider;
		}
	};
...
var isIE = (document.all) ? true : false;
	...
	Slider.prototype = {
	  ...
	  Move: function() {
		//添加tooltip
		tip.find(".slider-value").html(this.options.tipFt?this.options.tipFt(this.getValue()):this.getValue());
		if(this.options.notip){
			tip.remove();
		}else{
			this.Container.appendChild(tip.get(0));//设置dom 父子关系
		}
		...
		if(this._horizontal){
				var css1 ={
						top:"17px",
						left: "2px",
						"border-top": "4px solid rgb(149, 184, 231)",
						"border-right":""
				};
				var css2 ={
				top:"16px",
				...
				var css1 = {
						top:"2px",
						left: "-10px",
						"border-right": "4px solid rgb(149, 184, 231)",
						"border-top":""
			...
			$(".slider-tooltip-arrow").css(css1);
			$(".slider-tooltip-arrow-inner").css(css2);
		if(this._horizontal){
			tip.css({
				top : this.Bar.offsetTop-this.Bar.offsetHeight-10+'px',
				left : this.Bar.offsetLeft+'px'
				});
		}else{
			tip.css({
				top : this.Bar.offsetTop+'px',
				left :this.Bar.offsetWidth+10+'px'
				});
	  ...
	  WheelCtrl: function(e) {
		if(!$.support.leadingWhitespace){//判断IE8
	  	...
	  	if(e.wheelDelta){//IE/Opera/Chrome
			delta = 0 - e.wheelDelta / 120;
		}else if(e.detail){//Firefox
			delta = e.detail / 3;
		}
		var i = this.WheelSpeed * delta;
	  ...
	  getValue: function() {
		//根据最大最小值和滑动百分比取值
		return Math.round( this.MinValue + this.GetPercent() * (this.MaxValue - this.MinValue));
	  },
	  //设置值位置
	...
	Drag.prototype = {
	  ...
	  initialize: function(drag, options) {
		...
		this._Handle = $get(this.options.Handle) || this.Drag;
		this._mxContainer = $get(this.options.mxContainer) || null;
