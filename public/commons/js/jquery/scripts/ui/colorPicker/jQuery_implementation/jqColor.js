			...
			var renderCallback = function(colors, mode) {
				...
				doEventListeners = function(elm, multiple, off) {
					...
					$(elm)[onOff]('focus.colorPicker', function(e) {
						...
						//计算left 和 top，在右侧和下方边界时进行特殊处理 by wzw
						var bLeft = position.left + options.margin.left -  atrect.left;
						var bTop = position.top + $input.outerHeight(true) + options.margin.top - atrect.top;
						var ww = $(window).width();
						var wh = $(window).height();
						var cw = $colorPicker.width() > 158 ? $colorPicker.width():158; //应该为$colorPicker.width()，但是第一次渲染时并不能正确获得158px的宽度，暂时写一个固定值
						var ch =   173;//? $colorPicker.height():173;
						if(bLeft+cw > ww){
							bLeft -=  bLeft+cw - ww
						}
						if(bTop+ch > wh){
							bTop -=  bTop+ch - wh+$(this).outerHeight()+6
						}
						$colorPicker.css({
							'position': 'absolute',
							'left': bLeft,
