...
OpenApiClient = function(serverUrl, accessKey, secret) {
...
OpenApiClient.JSON = new (function() {
	...
	/**
	 * Returns true if the passed object is a JavaScript date object, otherwise
	 * false.
	 * 
	 * @param {Object}
	 *            The object to test
...
OpenApiClient.TIMESTAMP = "timestamp";
OpenApiClient.CMD = "cmd";
OpenApiClient.FORMAT = "format";
OpenApiClient.ACCESS_KEY = "access_key";
OpenApiClient.SIG = "sig";

OpenApiClient.prototype.sign = function(method, args, ignoreSign) {
	args = args || {};
	var t = {};
	for ( var attr in args) {
		...
		if (v != null && v != "") {
			if (typeof v == 'object') {
				t[attr] = OpenApiClient.JSON.encode(v);
			} else {
				t[attr] = v;
			}
		}
	...
	t[OpenApiClient.TIMESTAMP] = new Date().getTime();
	t[OpenApiClient.CMD] = method;
	t[OpenApiClient.FORMAT] = this.format;
	t[OpenApiClient.ACCESS_KEY] = this.accessKey;

	var sig = function(t, sec, ignore) {
		var keys = Object.keys(t).sort();
		var sigStr = sec;
		for (var i = 0; i < keys.length; i++) {
			var key = keys[i];
			var v = t[key];
			if (v != null && v != "" && ignore[key] == null) {
				sigStr += key + v;
			}
		}

		var hash = CryptoJS.HmacMD5(sigStr, sec);
		return hash.toString(CryptoJS.enc.Hex).toUpperCase();
	}
	t[OpenApiClient.SIG] = sig(t, this.secret, ignoreSign || {});
	return t;
}

OpenApiClient.prototype.jqCall = function(method, args, ok, err){
	args = this.sign(method, args);
